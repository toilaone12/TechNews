    ...
    <div class="container">
        <div class="row">
            <ol class="breadcrumb">
                ...
                @foreach($list_cate_by_id as $key => $cate_by_id)
                <li class="active"><a href="{{URL::to('/category-news',$cate_by_id->id_cate)}}">{{$cate_by_id->name_cate}}</a></li>
                @endforeach
            </ol>
        </div>
    </div>
...
<div class="entity_wrapper">
    ...
    <div class="entity_title header_purple">
        <h1><a href="{{URL::to('/category-news',$cate_by_id->id_cate)}}" target="_blank">{{$cate_by_id->name_cate}}</a></h1>
    </div>
    @endforeach
    <!-- entity_title -->
    ...
    <div class="entity_title">
        <a href="single.html" target="_blank"><h3> {{$news_new_by_cate->name_news}} </h3></a>
    </div>
    <!-- entity_title -->
    ...
    <ul class="pagination center-page">
        <li>
            ...
            <a  class="@if($pages > 1)
                    ...
                    @endif"
                href="@if($pages > 1)
                        {{URL::to('/category-news/'.$list_cate_by_id[0]->id_cate.'/'.$previous)}}
                    @else
                        {{URL::to('/category-news/'.$list_cate_by_id[0]->id_cate.'/'.$pages)}}
                    @endif" 
                aria-label="Previous"> <span aria-hidden="true">&laquo;</span> 
            </a>
        </li>
        @for($i = 1; $i <= $number; $i++)
        <li><a href="{{URL::to('/category-news/'.$list_cate_by_id[0]->id_cate.'/'.$i)}}">{{$i}}</a></li>
        @endfor
        <li>
            <?php
                $next = $pages + 1;
            ...
            <a  class="@if($pages < $number)
                ...
                href="@if($pages < $number)
                        {{URL::to('/category-news/'.$list_cate_by_id[0]->id_cate.'/'.$next)}}
                    @else
                        {{URL::to('/category-news/'.$list_cate_by_id[0]->id_cate.'/'.$pages)}}
                    @endif"  
